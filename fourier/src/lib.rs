...
pub fn create_fft_f32(size: usize) -> Box<dyn Fft<Real = f32> + Send> {
    ...
    if let Some(fft) = Autosort::<f32, Vec<Complex<f32>>>::new(size) {
        Box::new(fft)
    } else {
        Box::new(Bluesteins::<
            f32,
            Autosort<f32, Vec<Complex<f32>>>,
            Vec<Complex<f32>>,
        >::new(size))
    }
}
...
pub fn create_fft_f64(size: usize) -> Box<dyn Fft<Real = f64> + Send> {
    use fourier_algorithms::{Autosort, Bluesteins};
    use num_complex::Complex;
    if let Some(fft) = Autosort::<f64, Vec<Complex<f64>>>::new(size) {
        Box::new(fft)
    } else {
        Box::new(Bluesteins::<
            f64,
            Autosort<f64, Vec<Complex<f64>>>,
            Vec<Complex<f64>>,
        >::new(size))
    }
}
